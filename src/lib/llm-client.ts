...
// Don't cache the ZAI instance — the config file (/etc/.z-ai-config) can be
// refreshed by the sandbox at any time (new session token), and a cached
// instance would keep using the old token until the process restarts.
// Creating a new instance per call is cheap (~1ms) and always reads fresh config.
